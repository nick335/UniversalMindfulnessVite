...
const SearchPaginationController = ({ currentPage, TotalPages, path, searchQuery  }: props) => {
  ...
  return (
    <div className='flexCenter mt-16 font-sans'>
     <div className='flex items-center border-[0.125rem] border-black rounded-lg p-0'>
      <Link to={`${path}${prevPage}${searchQuery}`}>
      ...
      <Link to={`${
        disableNextPage ? `${path}${currentPage}${searchQuery}` : `${path}${nextPage}${searchQuery}`
      }`} >
