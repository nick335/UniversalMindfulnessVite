...
const MainBlog = () => {
  ...
  return (
    <div className="hidden lg:block lg:flex-[2] bg-bgBlogBox  pb-[1.24rem] rounded-[1.41044rem] mb-6 md:mb-10 lg:mb-0 lg:h-fit lg:cursor-pointer">
      <Link  className="w-full h-fit" to={`/blog/${actualContent.id}`}>
      ...
      <h4 className="pl-[0.71rem] lg:pl-[1.2rem]  text-headerSecondary mt-3 font-dmSans text-[0.98731rem] leading-4 font-bold ">
        
        {actualContent.sub_section}</h4>
      <p className="pl-[0.71rem] lg:pl-[1.2rem] font-semibold text-[1.12838rem] leading-[1.12838rem] mt-1.5 capitalize">
        <Link to={`/blog/${actualContent.id}`}>
          {actualContent.title}
        </Link>
      </p>
    </div>
